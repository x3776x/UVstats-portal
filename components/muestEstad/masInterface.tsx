...
export default function MASInterface() {
    ...
    const [observaciones, setObservaciones] = useState<{ id: string; valor: string }[]>([]);
    const [resultado, setResultado] = useState<ResultadoMAS | null>(null);
    ...
    const handleContinuar = () => {
        ...
        const nuevas = Array.from({ length: n }, (_, i) => ({ id: `obs-${i + 1}`, valor: '' }));
    ...
    const handleEstimar = () => {
        ...
        const res = calcularMAS(parametro, N, n, ic, valores, elementosNum);
        setResultado(res);
    };
    ...
    const handleCalcularTamano = () => {
        ...
        if (parametro === 'proporcion') {
            ...
            setTmResultado(calcularTamanoMuestraMAS('proporcion', N, B, ic, { proporcionEstimada: p }));
            return;
        }
        ...
        if (tmModoVarianza) {
            ...
            setTmResultado(calcularTamanoMuestraMAS(parametro, N, B, ic, { estimacionVarianza: v }));
        } else {
            ...
            setTmResultado(calcularTamanoMuestraMAS(parametro, N, B, ic, { amplitudVariacion: a }));
        }
    };
        ...
        <div className="space-y-8">
                ...
                <div className="p-6 bg-gray-50 rounded-lg border border-gray-200 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" value={tamanoPoblacion} onChange={(e) => setTamanoPoblacion(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none" min={1} />
                        </div>
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Tamaño de la muestra (n)</label>
                            <input type="number" value={tamanoMuestra} onChange={(e) => setTamanoMuestra(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none" min={1} />
                        </div>
                        <div>
                            ...
                            <div className="mt-4 p-4 bg-white border border-gray-200 rounded-lg space-y-3">
                                    ...
                                    {parametro === 'proporcion' ? (
                                        ...
                                        <>
                                            <div className="flex items-center gap-2">
                                                <input type="radio" checked={tmModoVarianza} onChange={() => setTmModoVarianza(true)} id="tmVar" />
                                                <label htmlFor="tmVar" className="text-sm text-gray-600">Estimación de varianza</label>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <input type="radio" checked={!tmModoVarianza} onChange={() => setTmModoVarianza(false)} id="tmAmp" />
                                                <label htmlFor="tmAmp" className="text-sm text-gray-600">Amplitud de variación</label>
                                ...
                                {tmResultado !== null && (
                                    <p className="text-sm text-gray-700 bg-blue-50 border-l-4 border-blue-500 p-3 rounded-r-lg">
                                        Se necesitan muestrear <span className="font-bold">{tmResultado}</span> observaciones para estimar {PARAMETRO_LABEL[parametro].toLowerCase()} con un {ic}% de confianza.
                                    </p>
                                )}
                            </div>
                        )}
            ...
            {parametro && tablaGenerada && (
                <div className="space-y-4 fade-in">
                    <div className="flex justify-between items-center">
                        <h3 className="font-semibold text-gray-800">Ingresar Observaciones (n = {tamanoMuestra})</h3>
                    ...
                    <div className="max-h-96 overflow-y-auto border border-gray-200 rounded-lg shadow-sm">
                        <table className="w-full text-sm text-left text-gray-500">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-100 sticky top-0">
                                <tr>
                                    <th className="px-6 py-3 w-1/2">Observación</th>
            ...
            {resultado && parametro && (
                <div className="mt-8 p-6 bg-white border border-gray-200 rounded-xl shadow-lg fade-in">
                    <h3 className="text-xl font-bold text-gray-800 mb-4 border-b pb-2">Resultados — Muestreo Aleatorio Simple</h3>
