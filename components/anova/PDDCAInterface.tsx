...
import { parseBifactorialFile } from '@/utils/fileParser';
import { calculatePDDCA, AnovaResultPDDCA, DataRowPDDCA } from '../../utils/anovaCalculator';

export default function PDDCAInterface() {
    ...
    const [tableData, setTableData] = useState<{id: string, factorA: number, factorB: number, repeticion: number, rendimiento: string}[]>([]);
    const [isTableGenerated, setIsTableGenerated] = useState(false);
    const [resultados, setResultados] = useState<AnovaResultPDDCA | null>(null);
    ...
    const handleGenerateTable = () => {
        ...
        if (isNaN(a) || isNaN(b) || isNaN(r) || a < 2 || b < 2 || r < 2) {
            showToast("Se requieren al menos 2 niveles para cada factor y 2 repeticiones.", "error");
            return;
        }
        ...
        <div className="space-y-8">
                ...
                <div className="p-6 bg-gray-50 rounded-lg border border-gray-200 space-y-3">
                    ...
                    <input
                        ...
                        onChange={async (e) => {
                            ...
                            if (!file) return;
                            try {
                                const extractedData = await parseBifactorialFile(file);
                                ...
                                const mappedData = extractedData.map(row => ({
                                    ...
                                    rendimiento: row.rendimiento === null ? '' : String(row.rendimiento)
                                }));
                                
                                setTableData(mappedData);
                                
                                const a = Math.max(...mappedData.map(d => d.factorA));
                                const b = Math.max(...mappedData.map(d => d.factorB));
                                const r = Math.max(...mappedData.map(d => d.repeticion));
                                
                                setNivelesA(a.toString());
                                setNivelesB(b.toString());
                                setRepeticiones(r.toString());
                                setIsTableGenerated(true);
                    ...
                    <div className="text-xs text-gray-500 mt-2">
                        <p>El Excel debe tener 4 columnas NUMÉRICAS en este orden exacto:</p>
                        <code className="bg-gray-200 px-2 py-1 rounded">Factor A (Parcela Principal) | Factor B (Sub-parcela) | Repetición | Rendimiento</code>
                    </div>
                </div>
            )}
            ...
            {method === 'manual' && !isTableGenerated && (
                <div className="p-6 bg-gray-50 rounded-lg border border-gray-200 space-y-4">
                    <h3 className="font-semibold text-gray-800">Parámetros de Parcelas Divididas (DCA)</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Factor A (Parcela Principal)</label>
                        ...
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Factor B (Sub-parcela)</label>
                        ...
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Repeticiones</label>
                            <input type="number" value={repeticiones} onChange={(e) => setRepeticiones(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" min={2} />
                        </div>
                    </div>
            ...
            {method === 'manual' && isTableGenerated && (
                <div className="space-y-4 fade-in">
                    <div className="flex justify-between items-center">
                        <h3 className="font-semibold text-gray-800">Ingresar Rendimiento ({parseInt(nivelesA) * parseInt(nivelesB) * parseInt(repeticiones)} datos)</h3>
                        ...
                        <table className="w-full text-sm text-left text-gray-500">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-100 sticky top-0">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 w-1/4">Repetición</th>
                            ...
                            <tbody>
                                {tableData.map((row) => (
                                    <tr key={row.id} className="bg-white border-b hover:bg-gray-50">
                                        <td className="px-6 py-3 font-medium text-gray-900">Nivel A {row.factorA}</td>
                                        <td className="px-6 py-3">Nivel B {row.factorB}</td>
                                        <td className="px-6 py-3">Rep. {row.repeticion}</td>
                        ...
                        <button className="bg-green-600 text-white px-6 py-2 rounded-md hover:bg-green-700"
                            onClick={() => {
                                try {
                                    ...
                                    const formattedData: DataRowPDDCA[] = tableData.map(row => ({
                                        ...row, rendimiento: parseFloat(row.rendimiento) || 0
                                    }));
                                    
                                    setResultados(calculatePDDCA(parseInt(nivelesA), parseInt(nivelesB), parseInt(repeticiones), formattedData));
                                } catch (error) { showToast("Ocurrió un error al calcular.", "error"); }
                            }}>
                            Calcular ANOVA PD-DCA
                        </button>
                    </div>
                </div>
            )}
            ...
            {resultados && (
                <div className="mt-8 p-6 bg-white border border-gray-200 rounded-xl shadow-lg fade-in">
                    <h3 className="text-xl font-bold text-gray-800 mb-4 border-b pb-2">Tabla ANOVA (Parcelas Divididas DCA)</h3>
                        ...
                        <table className="w-full text-sm text-left text-gray-700 border-collapse">
                            ...
                            <tbody>
                                ...
                                <tr className="border-b hover:bg-gray-50 bg-blue-50/30">
                                    <td className="px-4 py-3 font-bold text-blue-900" colSpan={7}>Parcelas Principales</td>
                                </tr>
                                <tr className="border-b hover:bg-gray-50">
                                    <td className="px-4 py-3 font-medium pl-8">Factor A</td>
                                ...
                                <tr className="border-b hover:bg-gray-50">
                                    <td className="px-4 py-3 font-medium pl-8">Error A</td>
                                    <td className="px-4 py-3 text-right">{resultados.glErrorA}</td>
                                    <td className="px-4 py-3 text-right">{resultados.scErrorA.toFixed(4)}</td>
                                    <td className="px-4 py-3 text-right">{resultados.cmErrorA.toFixed(4)}</td>
                                ...
                                <tr className="border-b hover:bg-gray-50 bg-green-50/30">
                                    <td className="px-4 py-3 font-bold text-green-900" colSpan={7}>Sub-parcelas</td>
                                </tr>
                                <tr className="border-b hover:bg-gray-50">
                                    <td className="px-4 py-3 font-medium pl-8">Factor B</td>
                                ...
                                <tr className="border-b hover:bg-gray-50">
                                    <td className="px-4 py-3 font-medium pl-8">Interacción A×B</td>
                                ...
                                <tr className="border-b hover:bg-gray-50">
                                    <td className="px-4 py-3 font-medium pl-8">Error B</td>
                                    <td className="px-4 py-3 text-right">{resultados.glErrorB}</td>
                                    <td className="px-4 py-3 text-right">{resultados.scErrorB.toFixed(4)}</td>
                                    <td className="px-4 py-3 text-right">{resultados.cmErrorB.toFixed(4)}</td>
                                    <td className="px-4 py-3 text-right">-</td>
                                    <td className="px-4 py-3 text-right">-</td>
                                    <td className="px-4 py-3 text-right">-</td>
                                </tr>
                    ...
                    <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div className="p-3 bg-blue-50 border-l-4 border-blue-500 rounded-r-lg">
                            <p className="text-sm text-blue-900"><b>Factor A (Principal):</b> {resultados.conclusionA}</p>
                        </div>
                        <div className="p-3 bg-green-50 border-l-4 border-green-500 rounded-r-lg">
                            <p className="text-sm text-green-900"><b>Factor B (Sub-parcela):</b> {resultados.conclusionB}</p>
                        </div>
